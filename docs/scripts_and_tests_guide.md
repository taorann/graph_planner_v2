...
| `scripts/run_rule_agent.py` | 统一的沙箱运行入口。按照命令行参数构建 `PlannerEnv`，选择规则或本地 LLM 规划器，并在 RepoEnv / Docker / FakeSandbox 后端之间切换。支持 `--report` 输出轨迹日志。 | `graph_planner.env.planner_env`, `graph_planner.runtime.sandbox`, `graph_planner.agents.common.chat`, `graph_planner.infra.config` |
| `scripts/train_planner_grpo.py` | 以单一 YAML (`configs/experiments/planner_grpo_4gpu.yaml`) 驱动 Planner-only GRPO 训练，自动注册 JSONL、预拉容器并配置 Ray runtime。 | `graph_planner.integrations.rllm`, `omegaconf`, `ray`, `graph_planner.infra.config` |
| `scripts/eval_planner_grpo.py` | 复用训练脚本的配置解析逻辑，加载指定 checkpoint 执行验证流程。 | `graph_planner.integrations.rllm`, `omegaconf`, `ray` |
| `scripts/register_graphplanner_dataset.py` | 将 RepoEnv 任务描述注册到 rLLM 数据集仓库，便于训练时引用。 | `graph_planner.integrations.rllm.dataset`, `R2E-Gym` |
...
- `scripts/prepare_datasets.py` 支持 `--skip-*` 与 `--prepull-*` 参数，可一次性生成训练/评测所需的 JSONL、实例与 docker manifest。【F:scripts/prepare_datasets.py†L12-L86】【F:scripts/prepare_datasets.py†L135-L214】
- `scripts/train_planner_grpo.py` 负责加载 YAML、注册数据、构造 Ray runtime，并将 Planner/CGM 路径注入环境变量后启动 GRPO 训练循环。【F:scripts/train_planner_grpo.py†L322-L468】
- `scripts/eval_planner_grpo.py` 复用了 `train_planner_grpo` 的配置处理，额外要求 `--ckpt` 指向待评估的 checkpoint 目录。
...
2. **回归测试**：
   ...
   ```
   若依赖项缺失，可先安装 `R2E-Gym` 或使用 `pip install -e ./R2E-Gym` 完成补齐；当缺少 Verl 依赖时，`tests/test_reward_manager_loading.py` 会自动跳过。
