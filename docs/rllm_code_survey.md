...
| `trainer/agent_trainer.py` | 高层封装，负责初始化 Ray、挂接 Verl PPO 训练任务并传入自定义 agent/env。【F:rllm/rllm/trainer/agent_trainer.py†L1-L79】 | CLI/示例使用该入口统一训练流程，内部调用 Verl `TaskRunner`。 | Graph Planner 训练脚本直接实例化 `AgentTrainer` 并传入自定义 agent/env/config。【F:scripts/train_planner_grpo.py†L424-L468】 |
...
* **训练入口**：`train_planner_grpo.py` 导入 `AgentTrainer`，在应用配置覆写与资源检查后触发训练；Ray 地址与资源控制沿用 rLLM 默认实现。【F:scripts/train_planner_grpo.py†L600-L619】
