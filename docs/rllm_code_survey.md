...
| `rllm/rllm/trainer/agent_trainer.py` | 高层封装，负责初始化 Ray、挂接 Verl PPO 训练任务并传入自定义 agent/env。【F:rllm/rllm/trainer/agent_trainer.py†L1-L79】 | CLI/示例使用该入口统一训练流程，内部调用 Verl `TaskRunner`。 | Graph Planner 训练链路会直接实例化 `AgentTrainer` 并注入自定义 agent/env/config，实现与 rLLM 示例相同的闭环。 |
...
* **训练入口**：统一的训练 CLI 将直接实例化 `AgentTrainer`，在配置覆写与资源检查后触发 GRPO 训练；Ray 地址与资源控制沿用 rLLM 默认实现。
